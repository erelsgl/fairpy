...
def optimal_envy_free(agentsList: AgentList, rent: float, budget: dict) -> (dict, dict):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for c in lst_SCC:
            ...
            inputs.append((AgentList(tempAgentC),new_rent,B_c))
        results = list(executor.map(lambda x: maximum_rent_envy_free(*x), inputs))
    ans_µc_pc = results
    ...
    µ={}
    with ThreadPoolExecutor(max_workers=4) as executor:
        # Use the map method of the thread pool to apply the update_mu function to each element of the inputs list in parallel
        inputs = [(i, j) for i in agentsList.agent_names() for j in ans_µc_pc]

        def update_mu(inputs):
            i, j = inputs
            µc = dict(j[1][0])
            if i in µc:
                µ[i] = µc[i]

        executor.map(update_mu, inputs)
